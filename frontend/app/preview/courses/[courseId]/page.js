  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      try {
        ...
        const { data: courseData } = await publicAxios.get(`/api/public/courses/${courseId}`);
        setCourse(courseData.data);
        
        // Fetch course modules with preview flag
        const { data: modulesData } = await publicAxios.get(`/api/public/modules/course/${courseId}?preview=true`);
        setModules(modulesData.data);
        
        // Fetch instructor info if available
        if (courseData.data.instructorId) {
          const { data: instructorData } = await publicAxios.get(`/api/public/instructors/${courseData.data.instructorId}`);
          setInstructor(instructorData.data);
        }
      } catch (err) {
        setError('Failed to load course data: ' + (err.response?.data?.message || err.message));
        console.error(err);
      } finally {
        setLoading(false);
      }
