...
export default function EditNewsClient({ initialNews, params }) {
  ...
  const [formData, setFormData] = useState({
    ...
    category: initialNews?.category || 'general',
    isPublished: Boolean(initialNews?.isPublished)
  });
  
   const [image, setImage] = useState(null);
  const [imagePreview, setImagePreview] = useState(null);
  const [currentImage, setCurrentImage] = useState(initialNews?.imageUrl || null);
  const [removeImage, setRemoveImage] = useState(false);
  
  // Loading state hanya true jika tidak ada data awal
  const [loading, setLoading] = useState(!initialNews);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState(null);
  const [notFound, setNotFound] = useState(!initialNews);
      ...
      getNewsById(id)
        .then(response => {
          if (!response.success) {
            ...
            setCurrentImage(newsData.imageUrl || null);
            setNotFound(false);
          }
        })
        .catch(err => {
          console.error('Error fetching news on client:', err);
          setNotFound(true);
