...
export default function CourseTestsClient({ initialTests, initialError, params }) {
  const { courseId } = params;
  
  const [tests, setTests] = useState(initialTests);
  const [loading, setLoading] = useState(!initialTests && !initialError);
  ...
  const handleDeleteTests = async () => {
    ...
    try {
      ...
      // Reset state to show the "Create Tests" view
      setTests({ hasTests: false, requirePreTest: false, preTest: {}, postTest: {} });
    } catch (err) {
  ...
  const handleToggleRequirePreTest = async () => {
    const originalRequirePreTest = tests.requirePreTest;
    try {
      // Optimistic UI update
      setTests(prev => ({ ...prev, requirePreTest: !prev.requirePreTest }));
      await axios.put(`/api/courses/${courseId}/tests`, { 
        requirePreTest: !originalRequirePreTest 
      });
    } catch (err) {
      // Revert on error
      setTests(prev => ({ ...prev, requirePreTest: originalRequirePreTest }));
              ...
              <div className="bg-blue-50 p-4 rounded-md">
                <div className="flex">
                  ...
                  <div className="ml-3">
                    ...
                    <div className="mt-4">
                      <button
                        onClick={handleToggleRequirePreTest}
