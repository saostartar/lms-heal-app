...
export default function CreateQuizClient({ initialModuleTitle, params }) {
  const { courseId, moduleId } = params;
  
  // Gunakan data awal dari props
  const [moduleTitle, setModuleTitle] = useState(initialModuleTitle || '');
  const [loading, setLoading] = useState(!initialModuleTitle); // Loading hanya jika tidak ada data awal
