...
export default function TakeQuizClient({
  ...
  const [quizAttempt, setQuizAttempt] = useState(initialAttempt);
  const [quizDetails, setQuizDetails] = useState(initialDetails);
  const [questions, setQuestions] = useState(initialQuestions || []);
  const [error, setError] = useState(initialError);
  const [loading, setLoading] = useState(!initialAttempt && !initialError);
  ...
  // useEffect untuk startAttempt sudah tidak diperlukan, data sudah ada dari server.
  // Inisialisasi timer saat komponen dimuat dengan data dari server
  useEffect(() => {
    if (quizDetails?.timeLimit && quizAttempt?.status === 'in_progress') {
      // Di sini kita bisa lebih canggih dengan menghitung sisa waktu sebenarnya
      // dari `startTime` di `quizAttempt`, tapi untuk sekarang kita mulai dari awal.
      setTimeLeft(quizDetails.timeLimit * 60);
    }
  }, [quizDetails, quizAttempt]);
  ...
  useEffect(() => {
    if (timeLeft === null || timeLeft <= 0 || submitted || isSubmitting) {
      if (timeLeft === 0 && !submitted && quizAttempt?.status === 'in_progress') {
        submitQuiz();
      }
      return;
    }
  ...
  const saveAnswer = useCallback(async (questionId, answerData) => {
    ...
    try {
      await axios.post(`/api/quiz-attempts/${quizAttempt.id}/questions/${questionId}/answer`, answerData);
    } catch (err) {
      console.error(`Error saving answer for question ${questionId}:`, err);
      setError(`Failed to save answer for question ${questionId}.`);
    } finally {
  ...
  const handleAnswerChange = (questionId, value, type) => {
    ...
    if (type === 'multiple_choice' || type === 'true_false') {
      newAnswerData = { selectedOptionId: parseInt(value) };
      updatedAnswers[questionId] = { selectedOptionId: parseInt(value), textAnswer: null };
    } else {
      newAnswerData = { textAnswer: value };
      updatedAnswers[questionId] = { selectedOptionId: null, textAnswer: value };
    }
    setAnswers(updatedAnswers);
    saveAnswer(questionId, newAnswerData);
  };
