  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch progress
        const progressPromise = axios.get(
          `/api/learner/progress/module/${moduleId}`
        );

        // Fetch quizzes for this module
        const quizzesPromise = axios.get(`/api/quizzes/module/${moduleId}`);

        const [moduleResult, courseResult, lessonsResult, progressResult, quizzesResult] =
        ...
        if (moduleResult.status === "fulfilled") {
          setModule(moduleResult.value.data.data);
        } else {
          console.error("Failed to fetch module:", moduleResult.reason);
        ...
        if (lessonsResult.status === "fulfilled") {
          setLessons(lessonsResult.value.data.data);
        } else {
          console.error("Failed to fetch lessons:", lessonsResult.reason);
        }

        if (progressResult.status === "fulfilled") {
          setProgress(progressResult.value.data.data);
        } else {
